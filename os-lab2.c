...
void signal_handler_SIGTSTP(int signal_number)
{
...
void readCommandFromUser(char *args[], int *hasAmp, int *argv)
{
    ...
    {
        if (argv==0|| hist_cnt==0)
        {
            printf("No commands in history.\n");
        }
        for(int i=hist_cnt-1; i>=0; i--){
            for(int idx=0; idx<MAXLINE; idx++){
                printf("%c", history[i][idx]);
                if(history[i][idx]=='\0') {break;}
            }
    ...
    int q=0;
    for(q=0; q<MAXLINE; q++){
        if(userCommand[q]=='\0') break;
        history[hist_cnt][q]=userCommand[q];
    }
    history[hist_cnt][q]='\0';
